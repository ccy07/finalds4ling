...
* In particular, I wonder what affects the acceptability of this case. Three factors are considered: 

  * whether or not two reflexives are identical 
  * whether or not a less specific reflexive precedes a more-specific reflexive (*ziji*-*taziji*; "blocking"hereafter)
...
* Three categorical independent variables; one ordinal dependent variable (a 7-point Likert scale).

* This study used Cumulative Link Mixed Models (or CLMMs), which will test both three fixed effects and random effects.

* Cumulative Link Mixed Models is for the data with the dependant variable as ordinal.
---
...
---
# The results--Dialectual background plot 
.pull-left[
...
---
# Fits cumulative link mixed models 
...
---
# The best model
```{r, 'regression2', echo=T,warnings=F, message=F}
summary(fm4)
```
...
* In this study, the rating score data were analyzed using cumulative link mixed model and experiment-wise alpha was set at 0.05.

* The model containing Blocking, Identity and dialectual background provided the best fit of the data.

* Blocking (z = -4.182, p < 0.001), Identity (z = 8.951, p < 0.001) and dialectual background (z = 16.760, p < 0.001) all have main effects on the final rating scores.

  * The sentences without blocking have higher score than the sentences with blocking.
...
* If we have ordinal dependent variable, the assumption dignostics still work?
