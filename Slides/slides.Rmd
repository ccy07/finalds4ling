...
```
---
# The results--the plot for Blocking

```{r, 'plot1', echo=F,warnings=F, message=F}
library("tidyverse")
df_tidy %>%
   group_by(Blocking) %>%
...
# The results--he plot for Identity

```{r, 'plot2', echo=F,warnings=F, message=F}
library("tidyverse")
df_tidy %>%
   group_by(Identidy) %>%
   summarise(mean_rating = mean(rating))
...
```
---
# The results--the plot for Dialectual background

```{r, 'plot3', echo=F,warnings=F, message=F}
library("tidyverse")
df_tidy %>%
   group_by(dia) %>%
   summarise(mean_rating = mean(rating))
